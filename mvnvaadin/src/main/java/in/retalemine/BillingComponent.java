...
public class BillingComponent extends CustomComponent {
	...
	private Component buildAddToCart() {
		...
		final ComboBox productPriceCB = new ComboBox("Price List", Arrays.asList(new Double[] {
				10.0, 20.0, 30.0, 40.0, 50.0 }));
		TextField qty = new TextField("Quantity");
		final ComboBox qtySuffixCB = new ComboBox("Qty Suffix List", Arrays.asList(new String[] {
				"pcs", "kg", "lt" }));
		...
		qtySuffixCB.setNewItemHandler(new NewItemHandler() {
			...
			public void addNewItem(String newQtySuffix) {
				qtySuffixCB.addItem(newQtySuffix.trim().toLowerCase().replaceAll("\\s+", ""));
				qtySuffixCB.setValue(newQtySuffix.trim().toLowerCase().replaceAll("\\s+", ""));
			}
		});
